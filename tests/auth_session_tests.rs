use notifiapp_protocol_core::{
    AuthSessionEvent, ParsedAuthResponse, PostcardCodec, ProtocolCodec, SessionAuthHandler,
};
...
async fn test_auth_session_events() {
    ...
    let response_parser = |resp| match resp {
        TestResponse::Ok { token } => ParsedAuthResponse::Success { token },
        TestResponse::Expired => ParsedAuthResponse::SessionExpired,
        TestResponse::Failed => ParsedAuthResponse::Unauthorized,
    };

    let handler = SessionAuthHandler::<TestAction, TestResponse, PostcardCodec>::new(
        login_builder,
        resume_builder,
        response_parser,
    );
